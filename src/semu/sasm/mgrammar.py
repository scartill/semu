...
local_store = (pp.Suppress("LSTORE") + reg_ref + id).setParseAction(
    lambda r: (MFPP.local_store, [reg_indices[r[0]], r[1]])
)

local_load = (pp.Suppress("LLOAD") + id + reg_ref).setParseAction(
    lambda r: (MFPP.local_load, [r[0], reg_indices[r[1]]])
)

# Macro constants
m_uint_const = pp.Regex("[0-9]+")

const_def = (pp.Suppress("CONST") + id + m_uint_const).setParseAction(
    lambda r: (MFPP.const_def, r)
)

const_load = (pp.Suppress("CLOAD") + refname + reg_ref).setParseAction(
    lambda r: (MFPP.const_load, [r[0], reg_indices[r[1]]])
)
...
func_var = func_var_def.setParseAction(
    lambda r: (MFPP.func_var, [r[0], reg_indices[r[1]]])
)
func_prologue = func_decl + pp.ZeroOrMore(func_var) + pp.Suppress('BEGIN')

func_epilogue = pp.Suppress('END').setParseAction(lambda r: (MFPP.end_func, r)) \
    + pp.Optional(comment)
